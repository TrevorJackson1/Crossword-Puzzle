...
function parseClueBox($arr){
	...
	$clueString = "";
	$numberOfElements = 0;
	array_push($arr, new PuzzleInfo(0,0,"","",0,0));
	while($inc < $sizeOfDoc)
	{
...
//Error checking based on user input
$ErrorCheck = 0;
for($i = 0; $i < $size; $i++){
	if($puzzInfo[$i]->direction == 0){
		$ErrorCheck = 1;
	}
}
if($ErrorCheck == 1){//Notify user
	echo "There was in error in creating your file... Please try again";
}else{//Create File
